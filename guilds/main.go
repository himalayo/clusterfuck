...
var (
	...
	events_cfg        = redis.Options{
		Addr:     os.Getenv("GUILDS_EVENTS_REDIS_ADDR"),
		Password: os.Getenv("GUILDS_EVENTS_REDIS_PASSWORD"),
		DB:       0,
	}
...
func main() {
	go data.loadGuildPartsFromDB(context.Background())
	...
	go func() {
		...
		res, err := Cfg.RegisterService(context.Background(), "guilds-service", configuration.RedisStreamData{
			Stream: "networking-events",
			Group:  "guilds-service",
			Instance: &redis.Options{
				Addr:     events_cfg.Addr,
				Password: events_cfg.Password,
				DB:       events_cfg.DB,
			},
		}, []int{813}, NetPub.RedisConfig)
		if err != nil {
	...
	NetPub = networking.NewNetworkingPublisher(&redis.Options{
		Addr:     os.Getenv("GUILDS_OUTGOING_REDIS_ADDR"),
		Password: os.Getenv("GUILDS_OUTGOING_REDIS_PASSWORD"),
		DB:       0,
	})
	...
	Net.SetRedisSubscriber(&redis.Options{
		...
		Password: events_cfg.Password,
		DB:       0,
	}, "networking-events", "guilds-service")
