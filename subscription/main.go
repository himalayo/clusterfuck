...
var (
	...
	events_cfg = redis.Options{
		Addr:     os.Getenv("SUBSCRIPTION_EVENTS_REDIS_ADDR"),
		Password: os.Getenv("SUBSCRIPTION_EVENTS_REDIS_PASSWORD"),
		DB:       0,
	}
...
func main() {
	...
	go func() {
		...
		res, err := Cfg.RegisterService(context.Background(), "subscription-service", configuration.RedisStreamData{
	...
	NetPub = networking.NewNetworkingPublisher(&redis.Options{
		Addr:     os.Getenv("SUBSCRIPTION_OUTGOING_REDIS_ADDR"),
		Password: os.Getenv("SUBSCRIPTION_OUTGOING_REDIS_PASSWORD"),
		DB:       0,
	})
	...
	Net.SetRedisSubscriber(&redis.Options{
		...
		Password: events_cfg.Password,
		DB:       0,
	}, "networking-events", "subscription-service")
